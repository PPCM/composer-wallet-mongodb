...
module.exports = class MongoDBWallet extends Wallet {
  ...
  constructor(options) {
    ...
    if (options.encrypted === true) {
      ...
      this.walletSchema.pre('save', function (next) {
        let wallet = this
        let cipher = crypto.createCipher(algorithm, key)
        // Only encrypt if the credentials are modified
        if (!wallet.isModified('valueBase64') && !wallet.isModified('valueString')) { return next() }
        // Encrypt the values
        if (wallet.valueBase64) {
          let valueBase64Encrypted = cipher.update(wallet.valueBase64, 'utf-8', 'hex')
          valueBase64Encrypted += cipher.final('hex')
          // Override the values
          this.valueBase64 = valueBase64Encrypted
        }
        if (wallet.valueString) {
          let valueStringEncrypted = cipher.update(wallet.valueString, 'utf-8', 'hex')
          valueStringEncrypted += cipher.final('hex')
          this.valueString = valueStringEncrypted
        }
        return next()
      })

      this.walletSchema.pre('find', function (next) {
        let wallet = this
        ...
        if (wallet.valueBase64) {
          ...
          // Override the values
          this.valueBase64 = valueBase64Decrypted
        }
        if (wallet.valueString) {
          let valueStringDecrypted = decipher.update(wallet.valueString, 'utf-8', 'hex')
          // Override the values
          this.valueString = valueStringDecrypted
        }
        return next()
      })
    }
