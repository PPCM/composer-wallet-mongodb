...
module.exports = class MongoDBWallet extends Wallet {
  ...
  async put(name, value) {
    ...
    if (value instanceof Buffer) {
      ...
      if (this.encrypted) {
        let valueBase64Encrypted = this.cipher.update(card.valueBase64, 'utf-8', 'hex')
        valueBase64Encrypted += this.cipher.final('hex')
        card.valueBase64 = valueBase64Encrypted
      }
    } else if (value instanceof String || typeof value === 'string') {
      card.valueString = value
      if (this.encrypted) {
        let valueStringEncrypted = this.cipher.update(card.valueString, 'utf-8', 'hex')
  ...
  async get(name) {
    ...
    if (card.hasOwnProperty('valueBase64')) {
      if (this.encrypted) {
        let valueBase64Decrypted = this.decipher.update(card.valueBase64, 'utf-8', 'hex')
        card.valueBase64 = valueBase64Decrypted
      }
      data = Buffer.from(card.valueBase64, 'base64')
    }
    else if (card.hasOwnProperty('valueString')) {
      if (this.encrypted) {
        let valueStringDecrypted = this.decipher.update(card.valueString, 'utf-8', 'hex')
        card.valueString = valueStringDecrypted
      }
